    ...
    methods(Static)
        ...
        function trace_to_fill = trace_TxD_2x2( config,H_trace_normalized,H_trace_interf_normalized,precoding_matrix,debug_output )
            ...
            parfor blockIdx=1:nBlocks
                ...
                size_A = size(A);
                size_C = size(C);
                H_tilde1_pinv_block = zeros([size_H(1) size_H(2) size_H(3) size(block_H,3)]);
                A_block = zeros([size_A(1) size_A(2) size_A(3) size(block_H,3)]);
                C_block = zeros([size_C(1) size_C(2) size_C(3) size(block_H,3)]);
                for TTI_ = 1:size(block_H,3)
                    TTI_H       = block_H(:,:,TTI_,:);
                    TTI_H_inter = block_H_interf(:,:,TTI_,:);
                    
                    [   H_tilde1_pinv_block(:,:,:,TTI_),...
                        A_block(:,:,:,TTI_),...
                        C_block(:,:,:,TTI_) ] = phy_modeling.channelTraceFactory_v1.calculate_TTI_params_TxD_2x2(TTI_H,TTI_H_inter,nTX,nRX,SC_samples);
                end
        ...
        function [H_tilde1_pinv A C] = calculate_TTI_params_TxD_2x2(H_trace_normalized,H_trace_interf_normalized,nTX,nRX,SC_samples)
        ...
        function [ trace_to_fill ] = trace_OLSM( config,H_trace_normalized,H_trace_interf_normalized,precoding_matrix,debug_output,nLayers )
            ...
            for TTI_ = 1:TTI_length
                ...
                [   H0F(:,:,:,TTI_)...
                    ...
                    H0F_pinv(:,:,:,TTI_)...
                    A(:,:,:,TTI_)...
                    C(:,:,:,TTI_) ] = phy_modeling.channelTraceFactory_v1.calculate_TTI_params_OLSM(TTI_H,TTI_H_inter,WDU,nRX,nLayers,SC_samples);
            end
            ...
            chi   = reshape(sum(abs(A).^2,2),nLayers,SC_samples,TTI_length) - zeta;                      % Represents inter-layer interference (0 for perfect channel knowledge)
        ...
        function [H0F H1F H0F_pinv A C] = calculate_TTI_params_OLSM(H_trace_normalized,H_trace_interf_normalized,WDU,nRX,nLayers,SC_samples)
        ...
        function [ trace_to_fill ] = trace_CLSM( config,H_trace_normalized,H_t,H_trace_interf_normalized,precoding_matrix,debug_output)
            ...
            parfor slice_num = 1:number_of_labs
                ...
                for TTI_idx = 1:length(TTI_vect)
                    ...
                    [PMI_tmp(slice_num).PMI(:,TTI_idx)] = LTE_feedback(TTI_H_t,nLayers,precoding_matrix.W,nTX,'ZF',wideband_precoding);
                end
            end
            ...
            parfor slice_num = 1:number_of_labs
                TTI_vect                        = lab_start_idxs(slice_num):lab_end_idxs(slice_num);
                TTI_params(slice_num).H0F       = zeros(nRX,nLayers,SC_samples,length(TTI_vect));
                TTI_params(slice_num).H1F       = zeros(nRX,nLayers,SC_samples,length(TTI_vect));
                TTI_params(slice_num).H0F_pinv  = zeros(nLayers,nRX,SC_samples,length(TTI_vect));
                TTI_params(slice_num).A         = zeros(nLayers,nLayers,SC_samples,length(TTI_vect));
                TTI_params(slice_num).C         = zeros(nLayers,nLayers,SC_samples,length(TTI_vect));
                ...
                for TTI_idx = 1:length(TTI_vect)
                    ...
                    if TTI_ >  config.feedback_channel_delay
                        ...
                        [   TTI_params(slice_num).H0F(:,:,:,TTI_idx)...
                            ...
                            TTI_params(slice_num).PMI_trace(:,TTI_idx)] = phy_modeling.channelTraceFactory_v1.calculate_TTI_params_CLSM(TTI_H,TTI_H_inter,precoding_matrix.W,nRX,nLayers,SC_samples,PMI_act);
                    end
                end
            end
            for slice_num = 1:number_of_labs
        ...
        function [H0F H1F H0F_pinv A C PMI] = calculate_TTI_params_CLSM(H_trace_normalized,H_trace_interf_normalized,W,nRX,nLayers,SC_samples,PMI)
            % H_trace_normalized(:,:,1,N_SCs)
            % H_trace_interf_normalized(:,:,1,N_SCs)
